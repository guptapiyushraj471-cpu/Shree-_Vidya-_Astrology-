/* js/index.js
   Utility script for navigation, theme, modals, widgets, and booking forms.
   Drop this file into js/index.js (root-level folder).
*/
...
(function () {
  ...
  const testimonials = [
    ...
    {
      text: `"Amazing experience — very clear remedies and instant results. Highly recommended."`,
      author: '- Meera S., Indore',
      rating: '★★★★★',
    },
    {
      text: `"Very compassionate and precise reading. Helped me choose the right career direction."`,
      author: '- Anil K., Bhopal',
      rating: '★★★★★',
    },
  ];
    ...
    const reply = await new Promise((resolve) => {
      setTimeout(() => {
        if (/book|appointment/i.test(text))
          resolve(
            'To book a consultation please use the form on the Contact section or call +91 72238 79166.'
          );
        else if (/price|cost|fee|charge/i.test(text))
          resolve(
            'Service prices are shown on each service card. For custom queries please contact +91 72238 79166.'
          );
        else
          resolve(
            'Namaste — I can help with booking, service info, or directions. For deeper queries please write to shrishrividhyaastrology@gmail.com.'
          );
      }, 600);
    });
  ...
  window.openServiceDetails = function openServiceDetails(id) {
    const svcMap = {
      kundali: {
        ...
        desc: 'Complete birth chart reading...',
        price: '₹2,100',
        duration: '45 mins',
      },
      vastu: {
        title: 'Vastu Consultation',
        desc: 'Home & office Vastu analysis...',
        price: '₹3,500',
        duration: '60 mins',
      },
      numerology: {
    ...
    if (bookBtn) bookBtn.setAttribute('href', 'contact.html');
    showElement(modal);
  };
  ...
  function renderProducts() {
    ...
    products.forEach((p) => {
      ...
      card.innerHTML = `<h4>${p.title}</h4><div>₹${p.price}</div>
        <div style="margin-top:8px"><button class="btn btn-primary add-to-cart" data-id="${p.id}">Add</button></div>`;
    ...
    openCartBtn.addEventListener('click', () => {
      ...
      items.innerHTML = cart.length
        ? cart
            .map(
              (it) =>
                `<div style="margin-bottom:8px"><strong>${it.title}</strong> x ${it.qty} — ₹${it.price}</div>`
            )
            .join('')
        : '<div>No items</div>';
  ...
  if (checkoutForm) {
    checkoutForm.addEventListener('submit', (ev) => {
      ...
      if (msg) msg.textContent = 'Thank you — your order has been placed.';
    ...
    document.getElementById('closeOrder')?.addEventListener('click', () => {
      hideElement(document.getElementById('orderModal'));
    });
  }
